...
class RecommendationService(
    ...
    def Recommend(self, request: RecommendationRequest, context: ServicerContext) -> RecommendationResponse:
        ...
        books_for_category: List = books_by_category[request.category]
        num_results: int = min(request.max_results, len(books_by_category))
        books_to_recommend: List = random.sample(books_for_category, num_results)

        return RecommendationResponse(recommendations=books_to_recommend)


def serve():
    server: Server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
